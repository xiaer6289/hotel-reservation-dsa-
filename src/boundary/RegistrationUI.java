...
public class RegistrationUI {
    ...
    private void displayMenu() {
        ...
        System.out.println("4. Assign Room & Check In Next Standard Guest");
    ...
    /**
     * Real-life walk-in flow:
     * guest identification -> existing loyalty lookup -> room request ->
     * immediate arrival time -> expected checkout -> Standard/VIP routing.
     */
    private void addWalkInRegistration() {
        ...
        if (guest != null) {
            ...
            System.out.println("\nGuest not found.");
            System.out.println("Registering a new guest.");

            String guestName = readAlphabeticString("Enter Guest Name: ");
            ...
            if (guest == null) {
                Utility.printError("Unable to register new guest.");
                return;
            }
        ...
        if (loyaltyProfile != null) {
            ...
            Utility.printSuccess(
                    "Existing loyalty member detected. Registration added "
                    + "to the VIP priority heap.");
            ...
            Utility.printSuccess(
                    "No loyalty membership detected. Registration added "
                    + "to the standard FIFO queue.");
            System.out.println("Registration ID: " + registrationId);
            System.out.println(
    ...
    private void viewWaitingQueue() {
        ...
        if (waitingCount == 0) {
            System.out.println("No walk-in registrations are waiting.");
            return;
        }
        ...
        System.out.println(
                "Total Waiting Registrations: " + waitingCount);
    }
    ...
    private void checkInNextStandardGuest() {
        ...
        Room[] suitableRooms
                = controller.getSuitableRoomsForNextStandard();
        ...
        for (int i = 0; i < suitableRooms.length; i++) {
            ...
            System.out.printf(
                    "%d. Room %-5s | Type: %-15s | Floor: %-3s | "
                    + "Capacity: %d | Status: %s%n",
                    i + 1,
                    room.getRoomNumber(),
        ...
        int choice;
        while (true) {
            choice = readInteger("Select Room: ");

            if (choice >= 0 && choice <= suitableRooms.length) {
                break;
            }

            Utility.printError("Invalid room selection.");
        }
        ...
        Utility.printSuccess(
                "Room assigned and guest checked in successfully.");
        System.out.println(
                "Registration ID: " + registration.getRegistrationId());
        System.out.println(
                "Confirmation Number: " + booking.getConfirmationNo());
        ...
        System.out.println(
                "Registration Status: " + registration.getStatus());
        System.out.println(
    ...
    private void searchRegistration() {
        ...
        String registrationId
                = readNonEmptyString("Enter Registration ID: ");
    ...
    private void cancelRegistration() {
        ...
        if (registration == null) {
            System.out.println("Waiting registration not found.");
            return;
        }

        Utility.printSuccess("Registration cancelled successfully.");
        System.out.println();
        System.out.println(registration);
    }
    ...
    private void displayVipAddError(int result) {
        switch (result) {
            case VipPriorityController.DUPLICATE_MEMBER_ID:
                Utility.printError(
                        "This loyalty member is already waiting in the VIP heap.");
                break;
    ...
    private String readAlphabeticString(String message) {
        while (true) {
            ...
            Utility.printError(
                    "Guest name can contain letters and spaces only.");
        }
    }
    ...
    private int readInteger(String message) {
        while (true) {
            ...
            try {
                ...
                Utility.printError("Please enter a valid number.");
            }
        }
    }
    ...
    private Long readPhoneNumber() {
        while (true) {
            String input = readNonEmptyString("Enter Phone Number: ");

            if (!Utility.isValidPhoneNo(input)) {
                Utility.printError(
                        "Invalid phone number. Enter 10 or 11 digits starting with 01.");
                continue;
            }
            ...
            try {
                ...
                Utility.printError("Phone number is too long.");
            }
        }
    }
    ...
    private LocalDateTime readDateTimeParts(String dateTimeType) {
        while (true) {
            System.out.println(
                    "\nEnter " + dateTimeType + " Date and Time");

            int year = readInteger("Enter Year (yyyy): ");
            int month = readInteger("Enter Month (1-12): ");
            int day = readInteger("Enter Day: ");
            int hour = readInteger("Enter Hour (0-23): ");
            int minute = readInteger("Enter Minute (0-59): ");
            ...
            try {
                ...
                Utility.printError(
                        "Invalid date or time. Please enter again.");
            }
        }
    }
}